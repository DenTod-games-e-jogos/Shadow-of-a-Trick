...
{
    [Tooltip("GameObject do tipo grid para a movimentação do aline.")]
    [SerializeField] Grid grid;
    [Tooltip("GameObject do tipo Tilemap que correspondente as paredes impedindo a movimentação do Alien.")]
    ...
    int[,] mapCosts;
    List<PathNode> route;
    [SerializeField] Vector3 nextDestinaton;
    ...
    private List<PathNode> CalculateRoute(Vector3Int endPos, Vector3Int startPos, Vector3Int offset)
    {
        ...
        while(openList.Count > 0)
        {
            ...
            foreach (PathNode neigbourNode in GetNeighbourList(currentNode, mapPaths))
            {
                ...
                if ((currentNode.gCost + actualgCost) < neigbourNode.gCost)
                {
                    ...
                    if (!openList.Contains(neigbourNode)) openList.Add(neigbourNode);
                } else
                {
    ...
    private List<PathNode> GetNeighbourList(PathNode currentNode, PathNode[,] mapPaths)
    {
        List<PathNode> neighbourList = new List<PathNode>();
        if (currentNode.coordinates.x - 1 >= 0)
        {
            if (mapPaths[currentNode.coordinates.x - 1, currentNode.coordinates.y] != null) 
                neighbourList.Add(mapPaths[currentNode.coordinates.x - 1, currentNode.coordinates.y]);
            if (currentNode.coordinates.y - 1 >= 0)
                if (mapPaths[currentNode.coordinates.x - 1, currentNode.coordinates.y - 1] != null) 
                    neighbourList.Add(mapPaths[currentNode.coordinates.x - 1, currentNode.coordinates.y - 1]);
        ...
        if (currentNode.coordinates.y - 1 >= 0)
            if (mapPaths[currentNode.coordinates.x, currentNode.coordinates.y - 1] != null) 
                neighbourList.Add(mapPaths[currentNode.coordinates.x, currentNode.coordinates.y - 1]);
        if (currentNode.coordinates.y + 1 < (max.y - min.y))
            if (mapPaths[currentNode.coordinates.x, currentNode.coordinates.y + 1] != null) 
                neighbourList.Add(mapPaths[currentNode.coordinates.x, currentNode.coordinates.y + 1]);
